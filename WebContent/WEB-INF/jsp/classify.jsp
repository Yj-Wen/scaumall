				...
				<div class="classify-top">
					<div class="classify">
						<ul class="classify-list">
							...
							<c:forEach items="${category.goodssecondarytypelist}" var="subcategory" varStatus="status">
				...
				<c:forEach items="${category.goodssecondarytypelist}" var="subcategory" varStatus="status">
					<div class="classify-main">
						<div class="classify-icon">
							<div>
								<img src="${subcategory.picture.picturePath} " alt="">
								<span>${subcategory.goodsSecondaryType}</span>
							</div>
							<p class="text-muted">${subcategory.goodsSecondaryTypeExplain}</p>
						</div>
						<div class="classify-items">
							<c:forEach items="${subcategory.goodsList}" var="good" varStatus="status">
						...
						function() {
							...
							if (which != -1) {
								if ((parseInt(which) % 4) != 0) {
									...
									var offset = $(".classify-items:eq(3)")
											.offset();
									console.log(offset.top);
									$("html,body").animate({
										scrollTop : offset.top - "170" + "px"
